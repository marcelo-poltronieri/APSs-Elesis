          ...
          */
          while (parser.advance()){
            switch (parser.commandType(parser.command())){
                case C_COMMAND:
                    String[] parts = parser.instruction(parser.command());
                    String compCode = Code.comp(parts);
                    String destCode = Code.dest(parts);
                    String jumpCode = Code.jump(parts);
                    instruction = "10" + compCode + destCode + jumpCode;
                    break;
                case A_COMMAND:
                    String symbol = parser.symbol(parser.command());
                    int address;
                    ...
                    instruction = "0" + String.format("%17s", Integer.toBinaryString(address)).replace(' ', '0');
                    break;
                default:
                    continue;
            }
